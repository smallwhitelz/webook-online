...
func NewFeedService(repo repository.FeedRepository, handlerMap map[string]Handler) FeedService {
...
// GetFeedEventListV2 支持活跃用户版本
func (f *feedService) GetFeedEventListV2(ctx context.Context, uid, timestamp, limit int64) ([]domain.FeedEvent, error) {
	var eg errgroup.Group
	// 因为并发操作
	var lock sync.Mutex
	events := make([]domain.FeedEvent, 0, limit*2)
