...
func (c *RewardRedisCache) SetCachedCodeURL(ctx context.Context, cu domain.CodeURL, r domain.Reward) error {
	key := c.codeURLKey(r)
	val, err := json.Marshal(cu)
	if err != nil {
		return err
	}
	// 如果你担心 30 分钟刚好是微信订单过期的问题，那么你可以设置成 29 分钟
	return c.client.Set(ctx, key, val, time.Minute*30).Err()
}

func (c *RewardRedisCache) codeURLKey(r domain.Reward) string {
	return fmt.Sprintf("reward:code_url:%s:%d:%d", r.Biz, r.BizId, r.Uid)
}
